...
const showComments = (id) => {
  ...
  const allComents = document.createDocumentFragment();
  ...
  contentDataArray[id].comments.map((element) => {
    ...
    allComents.append(oneComment);
  });

  //{id: 1, avatar: 'img/avatar-2.svg', message: name
...
const pressEscHandler = (evt) => {
  if (isEscapeKey(evt)) {
    ...
    closeModal(bigPictureSection); // не смог обойти, возможно можно передавать жтот параметр глобально
  }
};
...
const openBigPicture = (evt) => {
  ...
  if (evt.target.classList.contains('picture__img')) {
    ...
    socialComments.append(showComments(evt.target.getAttribute('id')));
  }
};
